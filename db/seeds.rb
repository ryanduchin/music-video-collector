u100 = User.create!(username: 'guest', password: 'password')
u1 = User.create!(username: 'ryanduchin', password: 'asdfasdf')
u2 = User.create!(username: 'Ken', password: 'asdfasdf2')
u3 = User.create!(username: 'Sasha', password: 'asdfasdf2')
u4 = User.create!(username: 'AleXandra', password: 'asdfasdf2')
u5 = User.create!(username: 'Carl', password: 'asdfasdf2')
u6 = User.create!(username: 'Allllan', password: 'asdfasdf2')
u7 = User.create!(username: 'Alex', password: 'password2')
u8 = User.create!(username: 'Allan', password: 'password2')
u9 = User.create!(username: 'Axer', password: 'asdfasdf2')
u10 = User.create!(username: 'MrMozo', password: 'asdfasdf2')
u11 = User.create!(username: 'Steve', password: 'asdfasdf2')
u12 = User.create!(username: 'Alex25', password: 'asdfasdf2')



v1 = u1.playlists.create!(name: 'House')
v2 = u1.playlists.create!(name: 'Trance')
v3 = u1.playlists.create!(name: 'Techno')
v4 = u3.playlists.create!(name: 'Pop')
v1 = u4.playlists.create!(name: 'House')
v1 = u3.playlists.create!(name: 'My Fav Songs')
v1 = u4.playlists.create!(name: 'My Favorite Songs')
v1 = u5.playlists.create!(name: 'Favorite Songs')
v1 = u6.playlists.create!(name: 'Favorite Songs')
v1 = u7.playlists.create!(name: 'Fav Songs')
v1 = u7.playlists.create!(name: 'For George')
v1 = u2.playlists.create!(name: 'Road Trip')
v1 = u6.playlists.create!(name: 'Road Trip')
v1 = u8.playlists.create!(name: 'Road Trip')
v1 = u9.playlists.create!(name: 'Road Trip')
v1 = u11.playlists.create!(name: 'Road Trip')
v1 = u12.playlists.create!(name: 'Road Trip')




p2 = u3.posts.create!(title: 'Peace of Mind', url: 'https://www.youtube.com/watch?v=eilZug1R4Es', artist: 'Above & Beyond', album: 'Group Therapy')
p3 = u3.posts.create!(title: 'Melba', url: 'https://www.youtube.com/watch?v=QYvZ8TUyn9w', artist: 'Sunny Lax')
p4 = u3.posts.create!(title: 'Ruby', url: 'https://www.youtube.com/watch?v=LH_15HVH5Yc', artist: 'Cirez D')
p5 = u2.posts.create!(title: 'Greyhound', url: 'https://vimeo.com/75755258', artist: 'Swedish House Mafia')
p6 = u3.posts.create!(title: 'Monument', url: 'https://www.youtube.com/watch?v=6c-RbGZBnBI', artist: 'Royskopp & Robyn')
p7 = u1.posts.create!(title: 'Dancing On My Own', url: 'https://www.youtube.com/watch?v=SW05tcG3Fgw', artist: 'Robyn', staff: true)
p8 = u4.posts.create!(title: 'Chasing Time', url: 'https://www.youtube.com/watch?v=jtTjzDTpx8o', artist: 'Azelia Banks')
p9 = u3.posts.create!(title: 'Generate', url: 'http://www.vevo.com/watch/eric-prydz/generate/GBUV71500138', artist: 'Eric Prydz', staff: true)
p10 = u1.posts.create!(title: 'We Are All We Need', url: 'https://www.youtube.com/watch?v=phXRX1p8woY', artist: 'Above & Beyond', album: 'Group Therapy', description: 'feat. Alex Vargas')
p11 = u4.posts.create!(title: 'Sticky Fingers', url: 'https://www.youtube.com/watch?v=qbtbZUmljDI', artist: 'Above & Beyond', album: 'Group Therapy')
p12 = u1.posts.create!(title: 'Hello', url: 'https://www.youtube.com/watch?v=c9thvmIOdnQ', artist: 'Above & Beyond', album: 'Group Therapy')
p13 = u4.posts.create!(title: 'Sun & Moon', url: 'https://www.youtube.com/watch?v=ll5ykbAumD4', artist: 'Above & Beyond')

q1 = u1.posts.create!(title: 'Sometimes I Feel So Deserted', url: 'https://www.youtube.com/watch?v=8OUqqsXVmlc', artist: 'The Chemical Brothers')
q2 = u4.posts.create!(title: 'Swoon', url: 'https://www.youtube.com/watch?v=CCp_3zw-CxA', artist: 'The Chemical Brothers')
q3 = u1.posts.create!(title: 'Escape Velocity', url: 'https://www.youtube.com/watch?v=efF8L9Jt_i8', artist: 'The Chemical Brothers')
q4 = u1.posts.create!(title: 'Out of Control', url: 'https://www.youtube.com/watch?v=6sOpbRL8R4g', artist: 'The Chemical Brothers')
q5 = u4.posts.create!(title: 'Galvanize', url: 'https://www.youtube.com/watch?v=Xu3FTEmN-eg', artist: 'The Chemical Brothers')
q6 = u1.posts.create!(title: 'Hey Boy Hey Girl', url: 'https://www.youtube.com/watch?v=tpKCqp9CALQ', artist: 'The Chemical Brothers')
q7 = u4.posts.create!(title: 'Do It Again', url: 'https://www.youtube.com/watch?v=UVrwzjtBHq0', artist: 'The Chemical Brothers')
q8 = u3.posts.create!(title: 'Star Guitar', url: 'https://www.youtube.com/watch?v=0S43IwBF0uM', artist: 'The Chemical Brothers', staff: true)
q9 = u3.posts.create!(title: 'Go', url: 'https://www.youtube.com/watch?v=LO2RPDZkY88', artist: 'The Chemical Brothers')
q10 = u8.posts.create!(title: 'All The Single Ladies', url: 'http://www.vevo.com/watch/beyonce/single-ladies-(put-a-ring-on-it)/USSM20803009', artist: 'Beyonce')
q11 = u8.posts.create!(title: 'Run The World (Girls)', url: 'https://www.youtube.com/watch?v=VBmMU_iwe6U', artist: 'Beyonce')
q12 = u8.posts.create!(title: 'We Found Love', url: 'https://www.youtube.com/watch?v=tg00YEETFzg', artist: 'Rihanna', description: 'ft. Calvin Harris')
q13 = u12.posts.create!(title: 'Pour It Up', url: 'https://www.youtube.com/watch?v=ehcVomMexkY', artist: 'Rihanna')

z1 = u2.posts.create!(title: 'Blame', url: 'https://www.youtube.com/watch?v=6ACl8s_tBzE', artist: 'Calvin Harris', description: 'ft. John Newman')
z2 = u2.posts.create!(title: 'Summer', url: 'https://www.youtube.com/watch?v=ebXbLfLACGM', artist: 'Calvin Harris')
z3 = u2.posts.create!(title: 'Wrecking Ball', url: 'https://www.youtube.com/watch?v=My2FRPA3Gf8', artist: 'Miley Cyrus')
z4 = u5.posts.create!(title: 'E.T.', url: 'https://www.youtube.com/watch?v=t5Sd5c4o9UM', artist: 'Katy Perry')
z5 = u5.posts.create!(title: 'This Is How We Do', url: 'https://www.youtube.com/watch?v=7RMQksXpQSk', artist: 'Katy Perry')
z6 = u11.posts.create!(title: 'Firework', url: 'https://www.youtube.com/watch?v=vlLgvQErn6o', artist: 'Katy Perry')
z7 = u3.posts.create!(title: 'Last Friday Night (T.G.I.F.)', url: 'https://www.youtube.com/watch?v=KlyXNRrsk4A', artist: 'Katy Perry')
z8 = u10.posts.create!(title: 'Wide Awake', url: 'https://www.youtube.com/watch?v=k0BWlvnBmIE', artist: 'Katy Perry')
z9 = u6.posts.create!(title: 'Part Of Me', url: 'https://www.youtube.com/watch?v=uuwfgXD8qV8', artist: 'Katy Perry')
z10 = u6.posts.create!(title: 'Roar', url: 'https://www.youtube.com/watch?v=CevxZvSJLk8', artist: 'Katy Perry')
z11 = u7.posts.create!(title: 'Birthday', url: 'https://www.youtube.com/watch?v=CEUg7OplvIQ', artist: 'Katy Perry')
z12 = u7.posts.create!(title: 'Dark Horse', url: 'https://www.youtube.com/watch?v=0KSOMA3QBU0', artist: 'Katy Perry')

y1 = u7.posts.create!(title: 'Royals', url: 'https://www.youtube.com/watch?v=nlcIKh6sBtc', artist: 'Lorde')
y2 = u8.posts.create!(title: 'I Knew You Were Trouble', url: 'https://www.youtube.com/watch?v=vNoKguSdy4Y', artist: 'Taylor Swift')
y3 = u8.posts.create!(title: 'Bad Blood', url: 'https://www.youtube.com/watch?v=QcIy9NiNbmo', artist: 'Taylor Swift', description: 'ft. Kendrick Lamar')
y4 = u8.posts.create!(title: 'Up In The Air', url: 'https://www.youtube.com/watch?v=y9uSyICrtow', artist: 'Thirty Seconds To Mars', staff: true)
y5 = u9.posts.create!(title: 'A Beautiful Lie', url: 'https://www.youtube.com/watch?v=4Kvd-uquuhI', artist: 'Thirty Seconds To Mars')
y6 = u9.posts.create!(title: 'The Kill (Bury Me)', url: 'https://www.youtube.com/watch?v=8yvGCAvOAfM', artist: 'Thirty Seconds To Mars')
y7 = u9.posts.create!(title: 'Lazy Eye', url: 'https://www.youtube.com/watch?v=Ekdq1jbZLFU', artist: 'Silversun Pickups')
y8 = u11.posts.create!(title: 'Phantasm', url: 'https://vimeo.com/90796762', artist: 'Flying Lotus')
y9 = u11.posts.create!(title: 'Zodiac Sh*t', url: 'https://vimeo.com/25737856', artist: 'Flying Lotus', staff: true, description: "featured on Adult Swim")
y10 = u11.posts.create!(title: "You're Dead", url: 'https://vimeo.com/103317489', artist: 'Flying Lotus')
y11 = u11.posts.create!(title: 'Kill Your Co-Workers', url: 'https://vimeo.com/15568767', artist: 'Flying Lotus')





j3 = PlaylistPost.create!(playlist_id: c2.id, post_id: p2.id)
j4 = PlaylistPost.create!(playlist_id: c2.id, post_id: p3.id)
j5 = PlaylistPost.create!(playlist_id: c3.id, post_id: p4.id)
j1 = PlaylistPost.create!(playlist_id: c1.id, post_id: p1.id)
j6 = PlaylistPost.create!(playlist_id: c1.id, post_id: p5.id)
j2 = PlaylistPost.create!(playlist_id: c1.id, post_id: p2.id)
j7 = PlaylistPost.create!(playlist_id: c4.id, post_id: p6.id)
j8 = PlaylistPost.create!(playlist_id: c4.id, post_id: p7.id)
j9 = PlaylistPost.create!(playlist_id: c4.id, post_id: p8.id)

# user 1 follows user 2
f1 = Follow.create!(follower_id: 1, followable_id: 2, followable_type: "User")
f2 = Follow.create!(follower_id: 1, followable_id: 4, followable_type: "Playlist")
f3 = Follow.create!(follower_id: 2, followable_id: 2, followable_type: "Playlist")
f4 = Follow.create!(follower_id: 2, followable_id: 3, followable_type: "Playlist")
